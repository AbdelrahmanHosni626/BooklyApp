import 'package:flutter/material.dart';

class ColorsManager {
  static const Color mainBlue = Color(0xFF100B20);
  static const Color grey = Color(0xFF757575);
  static const Color lightYellow = Color(0xFFFFDD4F);
  static const Color lightOrange = Color(0xFF344e41);
  static const Color lightRed = Color(0xFFEF8262);
  static const Color darkGrey = Color(0xFF707070);



  static const Color darkBlue = Color(0xFF242424);
  static const Color lightGrey = Color(0xFFC2C2C2);
  static const Color lighterGrey = Color(0xFFEDEDED);
  static const Color moreLightGrey = Color(0xFFFDFDFF);
}